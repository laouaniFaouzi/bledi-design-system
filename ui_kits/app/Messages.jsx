...
export function Messages({ NS }) {
  const { Icon, Avatar, Badge, Alert, Input, Button, EmptyState, VerifiedName } = NS;
  const threads = [
        ...
        {threads.map((t, i) => (
          <button key={t.who} type="button" onClick={() => setOpen(i)}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <VerifiedName name={t.who} verified markSrc="../../assets/bledi-b-mark.png" />
      ...
      <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
            ...
            <VerifiedName name={threads[open].who} verified markSrc="../../assets/bledi-b-mark.png" />
          </div>

...
import { Button, Badge, Chip, Alert, Icon, IconName, Avatar, VerifiedName } from '../../src/components';
...
function Tile({ icon, title, body }: { icon: React.ReactNode; title: React.ReactNode; body?: React.ReactNode }) {
  return (
...
export function Landing() {
  ...
  const objects: [IconName, string][] = [['key', 'Clés'], ['file', 'Documents'], ['glasses', 'Lunettes'], ['shirt', 'Vêtements oubliés'], ['briefcase', 'Petit sac'], ['gift', 'Objet familial'], ['suitcase', 'Petite valise déclarée']];
  ...
  const trust: [IconName, string][] = [['shield-check', 'Identité vérifiée'], ['camera', 'Objet déclaré avec photos'], ['check', 'Validation Bledi'], ['credit-card', 'Paiement sécurisé'], ['key', 'Remise par code'], ['star', 'Avis après transaction']];
        ...
        <div style={{ background: 'var(--gradient-panel)', borderRadius: 'var(--radius-2xl)', boxShadow: 'var(--shadow-panel)', color: 'var(--text-on-dark)', padding: 'var(--space-12)', display: 'grid', gridTemplateColumns: 'minmax(0,1.15fr) minmax(0,.85fr)', gap: 'var(--space-12)', alignItems: 'center' }}>
          ...
          <div style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-5)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-raised)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginTop: 'var(--space-4)' }}>
              <Avatar initial="K" />
              <div>
                <VerifiedName name="Karim B." verified markSrc={bMarkUrl} />
      ...
      <S>
        <div style={{ background: 'var(--gradient-panel)', color: 'var(--text-on-dark)', borderRadius: 'var(--radius-2xl)', padding: 'var(--space-12)', boxShadow: 'var(--shadow-panel)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-3)', marginTop: 'var(--space-6)' }}>
            {([['shield-check', 'Profil vérifié'], ['camera', 'Objets visibles avant acceptation'], ['credit-card', 'Rémunération claire']] as [IconName, string][]).map(([ic, l]) => (
